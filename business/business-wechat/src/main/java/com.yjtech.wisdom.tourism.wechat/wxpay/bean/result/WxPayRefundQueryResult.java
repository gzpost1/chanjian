...
/**
 * 微信支付-退款查询返回结果 Created by wuyongchong on 2019/9/17.
 */
@Data
...
public class WxPayRefundQueryResult extends BaseWxPayResult {
   ...
   * 组装生成退款记录属性的内容.
   */
  private void composeRefundRecords() {
    if (this.refundCount != null && this.refundCount > 0) {
      ...
      for (int i = 0; i < this.refundCount; i++) {
        ...
        refundRecord
            .setSettlementRefundFee(this.getXmlValueAsInt("xml/settlement_refund_fee_" + i));
  ...
  public static WxPayRefundQueryResult fromXML(String xml) {
    WxPayRefundQueryResult result = BaseWxPayResult.fromXML(xml, WxPayRefundQueryResult.class);
