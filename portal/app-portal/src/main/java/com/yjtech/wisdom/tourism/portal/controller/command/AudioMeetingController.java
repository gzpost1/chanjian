...
public class AudioMeetingController {
    ...
    public JsonResult login(@RequestBody @Valid AudioMeetingDto createDto) {
        ...
        /**
         *  1\加入会议时判断当前平台已经有该会议编号的会议正在举行，若无，则创建并加入该会议
         */
        //时间超过8小时 会议人数小于1 视为会议结束
        ...
        queryWrapper.le(AudioMeetingEntity::getNumber,1);
        queryWrapper.le(AudioMeetingEntity::getCreateTime, LocalDateTime.now().plusHours( 0 - Integer.valueOf(configService.selectConfigByKey(EventContants.AUDIO_MEETING_EXPIRE))));
