...
public class SysProfileController extends BaseController {
    ...
    public JsonResult uploadAvatar(@RequestParam("avatarPath") String avatarPath) {
        if (StringUtils.isNotBlank(avatarPath)) {
            LoginUser loginUser = tokenService.getLoginUser(ServletUtils.getRequest());
            if (userService.updateUserAvatar(loginUser.getUsername(), avatarPath)) {
                Map<String, Object> result = new HashMap<>();
                result.put("imgUrl", avatarPath);
                // 更新缓存用户头像
        ...
        return JsonResult.error("更新头像异常，请联系管理员");
    }
