...
import com.yjtech.wisdom.tourism.common.utils.ip.AddressUtils;
import com.yjtech.wisdom.tourism.common.utils.ip.IpUtils;
 ...
 */
@Component
public class ScreenTokenService {
    // 令牌自定义标识
    @Value("${token.header}")
     ...
     * 数据库令牌有效期配置参数
     */
    protected static final String TOKEN_EXPIRE_TIME_MINUTES_KEY = "token.expire_minutes";
    ...
    public int getExpireTime() {
        String expireTimeStr = configService.selectConfigByKey(TOKEN_EXPIRE_TIME_MINUTES_KEY);
        if (StringUtils.isEmpty(expireTimeStr)) {
            return expireTime;
        }

        return Integer.parseInt(expireTimeStr);
    }
     ...
     * @return 用户信息
     */
    public ScreenLoginUser getLoginUser(HttpServletRequest request) {
        // 获取请求携带的令牌
        String token = getToken(request);
        if (StringUtils.isNotEmpty(token)) {
            Claims claims = parseToken(token);
            // 解析对应的权限以及用户信息
            String uuid = (String) claims.get(Constants.SCREEN_LOGIN_USER_KEY);
            String userKey = getTokenKey(uuid);
            ScreenLoginUser user = redisCache.getCacheObject(userKey);
            return user;
        }
        return null;
    }
     ...
     * 设置用户身份信息
     */
    public void setLoginUser(ScreenLoginUser loginUser) {
     ...
     * @return 令牌
     */
    public String createToken(ScreenLoginUser loginUser) {
        ...
        claims.put(Constants.SCREEN_LOGIN_USER_KEY, token);
     ...
     * @return 令牌
     */
    public void verifyToken(ScreenLoginUser loginUser) {
     ...
     */
    public void refreshToken(ScreenLoginUser loginUser) {
     ...
     * @param loginUser 登录信息
     */
    public void setUserAgent(ScreenLoginUser loginUser) {
        UserAgent userAgent =
    ...
    private String getTokenKey(String uuid) {
        return Constants.SCREEN_LOGIN_USER_KEY + uuid;
    }
}
